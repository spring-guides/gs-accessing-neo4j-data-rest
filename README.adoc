...
You'll build a Spring application that let's you create and retrieve `Person` objects stored in a https://www.neo4j.org/[Neo4j] link:/understanding/NoSQL[NoSQL] database using Spring Data REST. Spring Data REST takes the features of https://projects.spring.io/spring-hateoas[Spring HATEOAS] and https://projects.spring.io/spring-data-neo4j[Spring Data Neo4j] and combines them together automatically.
...
The **people** object contains a list with Frodo. Notice how it includes a **self** link. Spring Data REST also uses https://www.atteo.org/2011/12/12/Evo-Inflector.html[Evo Inflector] to pluralize the name of the entity for groupings.
